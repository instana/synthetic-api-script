 ...
 */
function executeScript(scriptStr, scriptPath, bundlePath) {
  const vm2Global = {
    $http: require("request"),
    $env: $env,
    $synthetic: $env,
    $network: {
      setProxy: () => { helperMsg('setProxy'); }, // Used to set a proxy server to be used for all requests (HTTP, HTTPS).
      setProxyForHttp: () => { helperMsg('setProxyForHttp'); }, // Used to set a proxy server to be used for only HTTP requests.
      setProxyForHttps: () => { helperMsg('setProxyForHttps'); }, // Used to set a proxy server to be used for only HTTPs requests.
      clearProxy: () => { helperMsg('clearProxy'); }, // Used to remove the proxy configuration.
      getProxy: () => { helperMsg('getProxy'); }
    },
    $util: {
      secrets: {
        setURLSecretsRegExps: () => { helperMsg('setURLSecretsRegExps'); }
      },
      insights: {
        set: () => { helperMsg('$util.insights.set'); },
        get: () => { helperMsg('$util.insights.get'); }
      }
    },
    $attributes: {
      set: () => { helperMsg('$attributes.set'); },
      get: () => { helperMsg('$attributes.get'); }
    }
  };
 ...
 * @param  {String} scriptStr
 */
function executeScriptAPI(scriptStr) {
  executeScript(scriptStr, path.resolve('./myscript.js'), null);
}
